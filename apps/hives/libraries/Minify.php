	...
	public function js($js)
	{
 	...
 	{	
		...
		if (!$html) return true;
 		...
 		if (!function_exists('apc_fetch')) $ctype = 'file';
 		
		$this->CI->load->driver('cache',array('adapter' => $ctype));
		
/*
		$k = md5($html);
		
		if (!$c = $this->CI->cache->get($k))
		{
		$min = new Minify_HTML();
		$html = $min->minify($html);
		$this->CI->cache->save($k, $html, 300);
		} else {
		$html = $c;
		}
*/
		...
		$this->set_output('text/html',$html,0);
 	...
 	function gfiles($type,$id)
	{
		...
		if (!$url) $url = $this->CI->config->item('base_url');
	...
	private function set_output($type="",$content="",$time = false)
	{
		...
		//load header group if exist
		$header = $this->headers;
		
		
		if ($header)
		{
		$hdr = $this->CI->config->item('headers');
			...
			foreach ($hdr[$header] as $h)
				{
					...
					if (isset($h[1]))
						$h2 = $h[1];
					$this->CI->output->set_header($h1,$h2);				
				}
		}
		
/*
		echo "<pre>";
		print_r($this->CI->output->headers); exit;
*/
